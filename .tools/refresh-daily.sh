...
PYEOF
  ...
  echo "[7/7] npm run build — recompila o bundle com dados atualizados ..."
  cd "$ROOT/pepito-frontend"
  # Usa o node do PATH do sistema (nvm/volta/fnm)
  export PATH="$HOME/.volta/bin:$HOME/.nvm/versions/node/$(ls $HOME/.nvm/versions/node 2>/dev/null | tail -1)/bin:/usr/local/bin:$PATH"
  npm run build
